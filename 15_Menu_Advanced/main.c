...
static struct NewMenu nm_advanced[] =
{
    ...
    { NM_ITEM,  (STRPTR)"Show Grid", NULL,         CHECKIT | MENUTOGGLE,  0, (APTR)MID_SHOWGRID },
    { NM_ITEM,  (STRPTR)"Show Info", NULL,         CHECKIT | MENUTOGGLE,  0, (APTR)MID_SHOWINFO },
...
static int Menu_Create(struct App *app)
{
    ...
    app->menu = CreateMenusA(nm_advanced, create_tags);
    if (app->menu == NULL)
        return 0;
...
static void App_HandleMenuPick(struct App *app, UWORD code)
{
    ...
    while (code != MENUNULL)
    {
        ...
        if (item != NULL)
        {
            ...
            {
                case MID_SHOWGRID:
                    app->show_grid = ((item->Flags & CHECKED) != 0);
                    UI_UpdateTitle(app);
                    break;

                case MID_SHOWINFO:
                    app->show_info = ((item->Flags & CHECKED) != 0);
                    UI_UpdateTitle(app);
                    break;
